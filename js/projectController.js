function show1() {
    var x = document.getElementById("buscompanion");
    var y1 = document.getElementById("blockchain");
    var y2 = document.getElementById("lanechanger");
	var y3=document.getElementById("SpaceShooter");
	var y4=document.getElementById("Vehicle");
	var y5=document.getElementById("Robot");
	var y6=document.getElementById("Vista");
	var y7=document.getElementById("Mano");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
    if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		
    } 
    else {
        x.style.display = "none";
    }
}
function show2() {
    var x = document.getElementById("blockchain");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("lanechanger");
	var y3=document.getElementById("SpaceShooter");
    var y4=document.getElementById("Vehicle");
	var y5=document.getElementById("Robot");
	var y6=document.getElementById("Vista");
	var y7=document.getElementById("Mano");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
		var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
    } 
    else {
        x.style.display = "none";
    }
}
function show3() {
    var x = document.getElementById("lanechanger");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("SpaceShooter");
	var y4=document.getElementById("Vehicle");
	var y5=document.getElementById("Robot");
	var y6=document.getElementById("Vista");
	var y7=document.getElementById("Mano");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
    } 
    else {
        x.style.display = "none";
    }
}
function show4(){
	var x = document.getElementById("SpaceShooter");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("Vehicle");
	var y5=document.getElementById("Robot");
	var y6=document.getElementById("Vista");
    var y7=document.getElementById("Mano");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y4.style.display="none";
		y3.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
    } 
    else {
        x.style.display = "none";
    }
}
function show5(){
	var x = document.getElementById("Vehicle");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Robot");
	var y6=document.getElementById("Vista");
	var y7=document.getElementById("Mano");	
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
    if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
    } 
    else {
        x.style.display = "none";
    }
}
function show6(){
	var x = document.getElementById("Robot");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Vista");
    var y7=document.getElementById("Mano");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
	
    else {
        x.style.display = "none";
    }
}
function show7(){
	var x = document.getElementById("Vista");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Robot");
    var y7=document.getElementById("Mano");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
	
    else {
        x.style.display = "none";
    }
}
function show8(){
	var x = document.getElementById("Mano");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Robot");
    var y7=document.getElementById("Vista");
	var y8=document.getElementById("GymManager");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
	
    else {
        x.style.display = "none";
    }
}
function show9(){
	var x = document.getElementById("GymManager");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Robot");
    var y7=document.getElementById("Vista");
	var y8=document.getElementById("Mano");
	var y9=document.getElementById("HotelSystem");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
		
	
    else {
        x.style.display = "none";
    }
}
function show10(){
	var x = document.getElementById("HotelSystem");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Robot");
    var y7=document.getElementById("Vista");
	var y8=document.getElementById("Mano");
	var y9=document.getElementById("GymManager");
	var y10=document.getElementById("Crypto");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
		
	
    else {
        x.style.display = "none";
    }
}
function show11(){
	var x = document.getElementById("Crypto");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Robot");
    var y7=document.getElementById("Vista");
	var y8=document.getElementById("Mano");
	var y9=document.getElementById("GymManager");
	var y10=document.getElementById("HotelSystem");
	var y11=document.getElementById("Snake");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
		
	
    else {
        x.style.display = "none";
    }
}
function show12(){
	var x = document.getElementById("Snake");
    var y1 = document.getElementById("buscompanion");
    var y2 = document.getElementById("blockchain");
	var y3=document.getElementById("lanechanger");
	var y4=document.getElementById("SpaceShooter");
	var y5=document.getElementById("Vehicle");
	var y6=document.getElementById("Robot");
    var y7=document.getElementById("Vista");
	var y8=document.getElementById("Mano");
	var y9=document.getElementById("GymManager");
	var y10=document.getElementById("HotelSystem");
	var y11=document.getElementById("Crypto");
	if (x.style.display === "none") {
        x.style.display = "block";
        y1.style.display = "none";
        y2.style.display = "none";
		y3.style.display="none";
		y4.style.display="none";
		y5.style.display="none";
		y6.style.display="none";
		y7.style.display="none";
		y8.style.display="none";
		y9.style.display="none";
		y10.style.display="none";
		y11.style.display="none";
		}
		
	
    else {
        x.style.display = "none";
    }
}
